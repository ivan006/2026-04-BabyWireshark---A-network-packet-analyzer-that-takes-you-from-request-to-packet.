...
const FIELD_TIPS = {
src_port: 'The port number on the sender\'s machine this connection is coming from.',
...
flags: 'TCP control flags -- S=SYN (start), A=ACK (acknowledge), F=FIN (close), R=RST (abort), P=PSH (send now).',
window: 'How many bytes the receiver will accept before needing an acknowledgement. Controls flow.',
checksum: 'Error-detection value. If the receiver recalculates and it doesn\'t match, the packet is discarded.',
data_offset: 'How many 32-bit words are in the TCP header, so the receiver knows where the payload starts.',
...
fragment_offset:'If a large packet was split up, this says where this piece fits in the original.',
frame_type: 'The format used to wrap data at Layer 2. Ethernet II is the standard for modern networks.',
src_mac: 'Hardware address of the network card that sent this frame. MAC addresses are local to your network.',
dst_mac: 'Hardware address of the next hop (usually your router), not the final server.',
ethertype: 'Tells the receiver what Layer 3 protocol is inside. 0x0800 = IPv4, 0x0806 = ARP.',
...
payload_preview:'The raw payload. For HTTPS this will be encrypted and unreadable.',
};
...
if (flags === 'S' || flags === 'SA' || (flags === 'A' && !hasTLS && !hasRaw)) return 'TCP Handshake';
...
const tlsType = t.L5_L6_Session_Presentation?.type;
const isFromMe = src.startsWith('10.') || src.startsWith('192.168.') || src.startsWith('172.');
...
if (tlsType === 22 && isFromMe) return 'before we talk, let\'s agree on how to encrypt this &mdash; Client Hello';
if (tlsType === 22 && !isFromMe) return 'agreed, here\'s my certificate to prove who I am &mdash; Server Hello';
if (tlsType === 20) return 'switching to encrypted mode now &mdash; ChangeCipherSpec';
if (tlsType === 23 && isFromMe) return 'here\'s my request, but you can\'t read it &mdash; Application Data';
if (tlsType === 23 && !isFromMe) return 'here\'s my response, also encrypted &mdash; Application Data';
if (tlsType === 21) return 'alright I\'m done talking &mdash; TLS Alert';
if (flags.includes('F') && isFromMe) return 'I\'m done sending, close your end too &mdash; FIN';
if (flags.includes('F') && !isFromMe) return 'same, I\'m done too &mdash; FIN';
if (flags.includes('R')) return 'something went wrong, killing this connection &mdash; RST';
if (t.L7_Application) return 'here\'s the data you asked for &mdash; Application Data';
...
if (layers.L4_Transport) parts.push({ label: (layers.L4_Transport.protocol || 'tcp').toLowerCase(), color: '#86efac' });
if (layers.L5_L6_Session_Presentation) parts.push({ label: 'tls', color: '#67e8f9' });
if (layers.L7_Application) parts.push({ label: (layers.L7_Application.protocol ||
'app').toLowerCase().split('/')[0].split(' ')[0], color: '#a78bfa' });
return parts.map(p => {
...
const fields = [['Hostname', data.hostname], ['Resolved IP', data.ip], ['Port', data.port], ['Scheme', data.scheme],
['Path', data.path || '/']];
...
const fields = Object.entries(layers[l.key]).map(([k, v]) => {
const tip = FIELD_TIPS[k] || '';
return `
<tr>
  <td class="text-secondary pe-3 py-1"
    style="font-size:12px;white-space:nowrap;${tip ? 'cursor:help;text-decoration:underline dotted' : ''}" ${tip ?
    `title="${tip}" ` : '' }>${k}</td>
...
document.getElementById('modal-meta').textContent = `${pkt.size_bytes} bytes -- ${startTime ? ((pkt.timestamp -
startTime) * 1000).toFixed(1) + " ms" : "0 ms"}`;
...
const elapsed = startTime ? ((pkt.timestamp - startTime) * 1000).toFixed(1) : '0.0';
const phase = detectPhase(pkt);
const phaseColor = PHASE_COLORS[phase] || '#aaa';
